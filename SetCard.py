...
def set_card(coordinates, Karte, possible_coordinates, unavailable_coordinates, cards_set, orte, strassen, kloester, wiesen):
    ...
    for ort in Karte.orte_karte:
        ...
        for oeffnung in ort[1]:
            ...
            for o in orte:
                ...
                if dict[oe_to_check] in orte[o].koordinaten_plus_oeffnungen and \
                        oe_to_check in orte[o].koordinaten_plus_oeffnungen[dict[oe_to_check]]:
    ...
    if len(Karte.orte) > 0:
        update_ort(orte, liste_kanten, Karte, beteiligte_orte, coordinates[0], counter, wert_ort)
    ...
    return cards_set, possible_coordinates, unavailable_coordinates, orte, strassen, kloester, wiesen

def update_ort(liste_orte, liste_kanten_, Karte_, beteiligte_orte_, koordinaten_, counter_, wert_):
    ### alte Kanten loeschen

    for i in beteiligte_orte_:
        # print("i", i)
        liste_orte[i[0]].koordinaten_plus_oeffnungen[i[1]].remove(i[2])

    ### neue Orte erstellen

    for ort in Karte_.orte_karte:
        liste_orte.update({ort[0]: Ort(koordinaten_, ort[1])})
    ...
    if len(counter_) == 1:
        liste_orte[counter_[0]].koordinaten_plus_oeffnungen.update({koordinaten_: liste_kanten_})
        liste_orte[counter_[0]].wert += wert_
         ...
         if len(counter_) > 0:
            if Karte_.mitte == "O":
                ...
                liste_orte[hauptort].koordinaten_plus_oeffnungen.update({koordinaten_: liste_kanten_})
                liste_orte[hauptort].wert += wert_

                for ort in counter_:
                    liste_orte[hauptort].koordinaten_plus_oeffnungen.update(liste_orte[ort].koordinaten_plus_oeffnungen)
                    liste_orte[hauptort].wert += liste_orte[ort].wert
                    del liste_orte[ort]

            else:
                for ort in counter_:
                    liste_orte[ort].koordinaten_plus_oeffnungen.update({koordinaten_: []})
                    liste_orte[ort].wert += wert_
    return liste_orte
